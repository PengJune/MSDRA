...
def main():
    images_list1 = glob('imagess/T1-Transaxial-MRI-T2-PET/' + '/mri*.png')
    images_list2 = glob('imagess/T1-Transaxial-MRI-T2-PET/' + '/pet*.jpg')
    ...
    for i in enumerate(images_list1):
        ...
        with torch.no_grad():
            ...
            tem = tem1*abs(tem1/(tem0 + tem1)) + tem0*abs(tem0/(tem0 + tem1))
            #tem = tem1 * abs(tem1) + tem0 * abs(tem0)
            ...
            misc.imsave('T1-Transaxial-T2/' + name2[index], tem)
            index += 1
